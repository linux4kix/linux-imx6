...
void hdmi_mask_writeb(struct imx_hdmi *hdmi, u8 data, unsigned int reg,
		      u8 shift, u8 mask)
{
...
void hdmi_clk_regenerator_update_pixel_clock(struct imx_hdmi *hdmi)
{
				     ...
				     unsigned char addr)
{
	...
	if (val != data)
		dev_dbg(hdmi->dev, "%s: addr: 0x%08x 0x%04x != 0x%04x\n",
			__func__, addr, data, val);
	return (val == data);
}
